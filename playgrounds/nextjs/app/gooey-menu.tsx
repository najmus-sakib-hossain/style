...
export default function GooeyPage() {
    ...
    const menuAnimationStyle = {
        transform: isExpanded ? 'translateY(-240px) scale(1)' : 'translateY(0) scale(0)',
        opacity: isExpanded ? 1 : 0,
        transformOrigin: 'bottom center',
        transition: isExpanded
            ? 'transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.4s ease' // Faster opening
            : 'transform 1s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.3s ease',  // Slower closing
    };
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-center bg-black p-8">
            ...
            <div className="relative w-96 h-96 flex items-center justify-center">
                {/* Layer for the gooey effect shapes */}
                <Goo className="w-full h-full absolute flex items-center justify-center">
                    {/* Background shape for the menu */}
                    <div
                        className="absolute w-64 h-80 bg-[#0f0f0f] rounded-2xl"
                        style={menuAnimationStyle}
                    />
                    {/* Background shape for the circle */}
                    <div className="absolute w-24 h-24 bg-white rounded-full" />
                </Goo>

                {/* Layer for the clear UI content */}
                {/* The menu content (icons and text) - not affected by goo */}
                <div
                    className="absolute w-64 rounded-2xl p-4 flex flex-col gap-2 pointer-events-none"
                    style={menuAnimationStyle}
                >
                    {menuItems.map((item, index) => (
                        <div key={index} className="flex items-center gap-4 p-3 rounded-lg hover:bg-indigo-500 cursor-pointer pointer-events-auto">
                            {item.icon}
                ...
                {/* The main, clickable circle trigger, on top of everything */}
                <div
                    onClick={handleClick}
