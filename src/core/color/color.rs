...
use crate::core::color::{Error, utils::math::matrix_multiply};
#[cfg(not(feature = "std"))]
use alloc::{
    format,
...
mod tests {
    ...
    use crate::core::color::color::{
        Argb, Rgb, Xyz, delinearized, linearized, lstar_from_y, y_from_lstar,
    };
    #[cfg(not(feature = "std"))]
    use alloc::vec::Vec;
...
fn oklab_to_linear_srgb(oklab: Oklab) -> (f64, f64, f64) {
...
fn oklch_to_oklab(oklch: Oklch) -> Oklab {
    let a = oklch.c * (oklch.h.to_radians()).cos();
    let b = oklch.c * (oklch.h.to_radians()).sin();
...
impl From<Argb> for Oklab {
    fn from(argb: Argb) -> Self {
        let r = linearized(argb.red) / 100.0;
        let g = linearized(argb.green) / 100.0;
        let b = linearized(argb.blue) / 100.0;
...
impl From<Oklab> for Argb {
    fn from(oklab: Oklab) -> Self {
        ...
        let r_u8 = (delinearized(r * 100.0) as f64).clamp(0.0, 255.0) as u8;
        let g_u8 = (delinearized(g * 100.0) as f64).clamp(0.0, 255.0) as u8;
        let b_u8 = (delinearized(b * 100.0) as f64).clamp(0.0, 255.0) as u8;

        Argb::new(255, r_u8, g_u8, b_u8)
    }
}

impl From<Argb> for Oklch {
    fn from(argb: Argb) -> Self {
        let oklab = Oklab::from(argb);
